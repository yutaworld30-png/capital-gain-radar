...
# News/TDnet evidence remains the only input to the theme score. This mapping is
# a clearly labelled fallback so every company can still be browsed by business
# area without turning a broad JPX industry label into an investment signal.
...
def industry_theme(industry: object) -> str:
    normalized = str(industry or "").strip()
    return INDUSTRY_THEME_MAP.get(normalized, "その他事業（業種参考）")
